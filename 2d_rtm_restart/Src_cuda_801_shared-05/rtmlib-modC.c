...
if(GPU_start_step>=0){
		...
		rtm_gpu_func(GPU_start_step, nt, nz, nx, ex_Vx0, ex_Vz0, ex_sigmaxx0, ex_sigmazz0, ex_sigmaxz0, ex_m1_x, ex_m1_z, ex_aux_m2_c, ex_aux_m3_c, ex_aux_m2m3_c, debug, gpu_kernel_time_ps);
...
if((GPU_start_step < nt-3) && (GPU_start_step >=0)){
	...
	ctime = (ctime_ps * (GPU_start_step+1));
	speedup = ctime_ps/gpu_kernel_time_ps[0];
	fprintf(stderr, "CPU time per step %.8f\n",ctime_ps);
	fprintf(stderr, "GPU time per step %.8f\n",gpu_kernel_time_ps[0]);
	fprintf(stderr, "SPEEDUP Pure Kernel Computing: %.2f\n", speedup);
	speedup = ctime/gtime;
	fprintf(stderr, "SPEEDUP Application for %d Steps: %.2f\n", GPU_start_step+1, speedup);
