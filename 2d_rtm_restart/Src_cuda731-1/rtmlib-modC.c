...
if(it <= GPU_start_step){

		fprintf(stderr, "\nit = %d\n", it);
		gettimeofday(&start2, NULL);
		rtm_gpu_func(GPU_start_step, nt, nz, nx, Vx0, Vz0, sigmaxx0, sigmazz0, sigmaxz0, ex_m1_x, ex_m1_z, ex_aux_m2_c, ex_aux_m3_c, ex_aux_m2m3_c, debug);
...
//	
//		//*********Lin for debug**************//
//		if(fabs(debug[index3d(iz,ix  ,it)] - sigmazz0[index3d(iz,ix  ,it)]) > 10e-30 ){
//			fprintf(stderr,"ix=%d, iz=%d, cpu = %.30f\n", ix,iz,sigmazz0[index3d(iz,ix  ,it)]);
//			fprintf(stderr,"ix=%d, iz=%d, gpu = %.30f\n\n", ix,iz,debug[index3d(iz,ix  ,it)]);}
//		sigmazz0[index3d(iz,ix  ,it)]	=  debug[index3d(iz,ix  ,it)] ;
...
gettimeofday(&end1, NULL);
	...
	ctime_ps = ctime/(nt-GPU_start_step-1);
