...
module rtm_mod_adj
  ...
  real, allocatable, dimension(:,:), private :: m1_x,m1_z, aux_m3_c, aux_m2_c, aux_m2m3_c
  ...
  subroutine rtm_init(nz_in,dz_in,nx_in,dx_in,dt_in,xsource_in,zsource_in,zrec_in,fp_in,nt_in,abc_in,free_surface_in,m1_in,m2_in,m3_in,lame_in)
    ...
    allocate(m1_x(nz,nx))
    allocate(m1_z(nz,nx))
    allocate(aux_m2_c(nz,nx))
    allocate(aux_m3_c(nz,nx))
    allocate(aux_m2m3_c(nz,nx))
    ...
    call rtmmod_init(nz,nx,dx,dt,m1_x,m1_z,m2,m3)

    !Lin compute aux_c variables according to rtmmod_int() and will later be
    !used for C
    aux_m2_c = (dt/dx)* m2
    aux_m3_c = (dt/dx)* m3
    aux_m2m3_c = (dt/dx)*(m2+2.0*m3)

 !   call rtmmod_initF(nz,nx,dx,dt,m2,m3)
...
!    write(0,*)"0%           25%            50%           75%            100%"
!    write(0,'(a)',advance="no")"["
    call rtm_op3(nt, nz, nx, zrec,Vx0, Vz0, sigmaxx0, sigmazz0, sigmaxz0, Vx_in, Vz_in, sigmaxx_in, sigmazz_in, sigmaxz_in,m1_x, m1_z, aux_m2_c, aux_m3_c, aux_m2m3_c)
!    !Zero arrays
!    Vx0       = 0.
!    Vz0       = 0.
!    sigmaxx0  = 0.
