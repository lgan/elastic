...
if(it > nt-5){
	...
	for(iit=0;iit<nt;iit++)
		for(ix=4;ix<nx-5;ix++)
			for(iz=0;iz<nz;iz++){	
				ex_sigmaxx0[index3d_ex(iz,ix,iit)]=sigmaxx0[index3d(iz,ix,iit)];
	...
	//rtm_gpu_func(it, nt, nz, nx, Vx0, Vz0, sigmaxx0, sigmazz0, sigmaxz0, m1_x, m1_z, aux_m2_c, aux_m3_c, aux_m2m3_c);
        for(ix=0; ix<nx; ix++)
            for(iz=4; iz<nz-5; iz++){
              Vx0[index3d(iz,ix  ,it)] = Vx0[index3d(iz,ix  ,it)]	+ ex_aux_m2m3_c[index_ex(iz,ix-5)]*c1*ex_sigmaxx0[index3d_ex(iz,ix-5,it+1)]							
